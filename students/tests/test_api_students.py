...
def test_create_student(api_client):
    ...
    data = {"first_name": "Alice", "last_name": "Smith", "student_id": "STU123"}
...
def test_update_student_api(api_client):
    ...
    student = Student.objects.create(
        first_name="Old", last_name="Name", student_id="STU1"
    )

    url = reverse("student-detail", args=[student.id])

    response = api_client.patch(url, {"first_name": "New"})
...
def test_delete_student_api(api_client):
    ...
    student = Student.objects.create(
        first_name="Delete", last_name="Me", student_id="STU9"
    )
